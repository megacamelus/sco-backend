...
func run(ctx context.Context, log *logger.Logger) error {
	...
	cfg := struct {
		conf.Version
		Web struct {
			DebugHost string `conf:"default:0.0.0.0:9003"`
		}
		Expvar struct {
			Host            string        `conf:"default:0.0.0.0:9001"`
		...
		Prometheus struct {
			Host            string        `conf:"default:0.0.0.0:9002"`
			Route           string        `conf:"default:/metrics"`
			ReadTimeout     time.Duration `conf:"default:5s"`
			WriteTimeout    time.Duration `conf:"default:10s"`
			IdleTimeout     time.Duration `conf:"default:120s"`
		...
		Collect struct {
			From string `conf:"default:http://localhost:8083/debug/vars"`
		}
		Publish struct {
